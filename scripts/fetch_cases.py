...
def fetch_case_list(client: LawApiClient, page: int = 1, display: int = 20, dat_src_name: str = DEFAULT_DATA_SOURCE) -> List[Dict[str, str]]:
    ...
    for case in root.findall('.//prec'):
        ...
        rows.append({
            ...
            'source_url': build_public_url(BASE_SEARCH_URL, {'target': 'prec', 'type': client.response_type, 'page': page, 'display': display, 'datSrcNm': dat_src_name}),
        })
    return rows
...
def fetch_case_detail(client: LawApiClient, precedent_id: str) -> Dict[str, object]:
    ...
    return {
        ...
        'request_type': 'detail',
        'collected_at': timestamp,
        'source_url': build_public_url(BASE_SERVICE_URL, {'target': 'prec', 'type': client.response_type, 'ID': precedent_id}),
    }
